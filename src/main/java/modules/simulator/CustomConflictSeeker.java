    ...
    private void setBlockingStatus() {
        try {
            ...
            ResultSet rs = connection.createStatement().executeQuery(
                    "SELECT custom_announcement_id, MAX (blocking_status) FROM custom_conflicts GROUP BY custom_announcement_id");
            while (rs.next()){
...
//                                "  ris_peers BIGINT NOT NULL,\n" +
                ...
                ");\n");
        connection.createStatement().execute("CREATE TRIGGER set_timestamp BEFORE UPDATE ON custom_announcements FOR EACH ROW EXECUTE PROCEDURE update_timestamp()");
        ...
        connection.createStatement().execute("CREATE TABLE custom_conflicts (\n" +
                ...
                ")");
        connection.createStatement().execute("CREATE TRIGGER set_timestamp BEFORE UPDATE ON custom_conflicts FOR EACH ROW EXECUTE PROCEDURE update_timestamp()");
    }
